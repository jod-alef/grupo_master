...
from .models import (
    Empresa, Soldador, SolicitacaoCadastroSoldador,
    EnsaioMecanicoDobramento, EnsaioUltrassom, Raqs, Operador
)
class BulmaMixin:
...
class SolicitacaoCadastroSoldadorForm(BulmaMixin, forms.ModelForm):
    class Meta:
        ...
        fields = [
            'eps', 'norma_projeto', 'sinete',
            'processo_soldagem', 'consumivel_spec', 'consumivel_classificacao',
            'consumivel_diametro', 'metal_base_spec', 'metal_base_espessura',
            'metal_base_diametro', 'posicao_soldagem', 'posicao_soldagem_progressao',
            'cobre_junta', 'gas_protecao', 'purga', 'modo_transferencia',
            'ensaio'
        ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        metal_base_spec = self.initial.get('metal_base_spec', self.data.get('metal_base_spec'))
        ...
        if metal_base_spec in ["A-36", "SB536", "A-309", "A-312"]:  # Para chapas
            ...
            self.fields['posicao_soldagem'].choices = [
                choice for choice in self.fields['posicao_soldagem'].choices
    ...
    def clean(self):
        ...
        if metal_base_spec in ["A-36", "SB536", "A-309", "A-312"] and not metal_base_espessura:
...
class EnsaioMecanicoDobramentoForm(BulmaMixin, forms.ModelForm):
    class Meta:
        ...
        fields = [
            'solicitacao', 'fotos_amostra_1',
            'fotos_amostra_2', 'fotos_amostra_3', 'aprovado'
        ]
...
class RaqsForm(BulmaMixin, forms.ModelForm):
    class Meta:
        model = Raqs
        fields = ['solicitacao', 'lista_de_verificacoes']
...
class AdminEmpresaAdmin(UserAdmin):
    ...
    fieldsets = (
        ...
        ('Informações Pessoais', {'fields': ('first_name', 'last_name', 'email', 'empresa')}),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('username', 'email', 'empresa', 'password1', 'password2', 'is_staff', 'is_active'),
        }),
    )
